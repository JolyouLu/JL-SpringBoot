package top.jolyoulu.modules.apiversionmodule.apiverion;
...
public class ApiCondition implements RequestCondition<ApiCondition> {
    ...
    /**
     * 将此条件与另一个条件(如来自类型级和方法级{@code @RequestMapping}注释。
     *  @param其他条件组合。
     *  @return一个请求条件实例，它是组合的结果两个条件实例。
     */
    @Override
     ...
     * 获取符合条件的实例
     * 检查条件是否与可能返回新内容的请求匹配
     * 为当前请求创建的实例。例如一个条件多个URL模式可能只返回带有这些模式的新实例匹配请求。
     * 对于CORS的跨域请求，条件应该匹配潜在的，实际请求(例如URL模式、查询参数和HTTP方法从“Access-Control-Request-Method”头)。如果一个条件不能匹配一个飞行前的请求，它应该返回一个实例空内容，因此不会导致匹配失败。
    ...
    /**
     * 将这种情况与上下文中的另一种情况进行比较一个特定的要求。
     * 此方法假设两个实例都有通过{@link #getMatchingCondition(HttpServletRequest)}获得以确保他们只有当前请求相关的内容。
     */
